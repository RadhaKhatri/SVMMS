...
const addPart = async () => {
  ...
  quantity_used: partQty,
    ...
    toast({
      ...
      description:
        err.response?.data?.message || "Inventory not available",
      variant: "destructive",
    });
  }
};
        ...
        <div className="flex items-center justify-between">
          <div>
            <div className="flex items-center gap-4 mb-2">
              <h1 className="text-3xl font-bold text-primary">Job card {jobCard.id}</h1>
              <Badge className={statusConfig.className}>
                {statusConfig.label}
              </Badge>
            </div>
            <p className="text-muted-foreground text-lg">{jobCard.service_type}</p>
          </div>
          ...
          <Card className="bg-card/50 bg-white/10">
            ...
            <CardContent className="space-y-3">
              ...
              <div className="text-sm text-muted-foreground">
                Model: <span className="font-mono text-primary">{jobCard.vehicle.model}</span>
              </div>
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <Card className="bg-card/50 bg-white/10">
            ...
            <CardContent className="space-y-3">
              <div>
                ...
                <span className="font-medium text-foreground">{jobCard.timeline.start_time
  ? new Date(jobCard.timeline.start_time).toLocaleString()
  : "Not started"}</span>
              </div>
              <div>
                <span className="text-sm text-muted-foreground">Est. Completion: </span>
                <span className="font-medium text-primary">{jobCard.timeline.preferred_date}</span>
              </div>
              <div className="pt-2">
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <div className="text-sm text-muted-foreground">
                            Qty: {part.quantity} | Stock: <span className="text-primary">{part.quantity_used}</span> available
                          </div>
                        </div>
                      </div>
                      <div className="flex items-center gap-4">
                        <span className="font-bold text-lg text-primary">₹{Number(part.unit_price).toFixed(2)}</span>
