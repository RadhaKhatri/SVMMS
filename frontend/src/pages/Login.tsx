...
import { Settings, ShieldCheck, User, Wrench, X } from "lucide-react";
...
const Login = () => {
  ...
  const handleLogin = async (e: React.FormEvent, selectedRole: string) => {
    ...
    const email = (
      document.getElementById(`email-${selectedRole}`) as HTMLInputElement
    ).value;
    const password = (
      document.getElementById(`password-${selectedRole}`) as HTMLInputElement
    ).value;
          ...
          <CardContent className="pt-4">
            <Tabs value={role} onValueChange={setRole} className="w-full">
              <TabsList className="grid w-full grid-cols-4 bg-secondary/50">
                {(["customer", "mechanic", "manager", "admin"] as const).map(
                  (roleType) => {
              ...
              {(["customer", "mechanic", "manager", "admin"] as const).map(
                (roleType) => (
                  <TabsContent key={roleType} value={roleType} className="mt-6">
                    <form
                      onSubmit={(e) => handleLogin(e, roleType)}
                      className="space-y-5"
                    >
                      <div className="space-y-2">
                        <Label
                          htmlFor={`email-${roleType}`}
