...
const [isEditOpen, setIsEditOpen] = useState(false);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await axios.delete(`http://localhost:5000/api/vehicles/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setMessage(res.data.message);
      toast({
  ...
  const fetchServices = async (vehicleId) => {
    try {
      const res = await axios.get(`http://localhost:5000/api/vehicles/${vehicleId}/services`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  <DialogContent className="bg-black/100 max-w-xl border border-white">
    ...
    <form onSubmit={handleSubmit} className="space-y-3 mt-2 ">
      {["vin","make","model","engine_type","year","mileage"].map((field) => (
        <div key={field}>
    ...
    <form onSubmit={handleSubmit} className="space-y-3 mt-2">
      ...
      <Button type="submit" className="w-full gradient-primary">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {vehicles.map((v) => (
            <Card key={v.id} className="bg-card/50 hover:shadow-lg hover:shadow-primary/10 transition-all">
              ...
              <CardContent className="space-y-3 text-sm">
                ...
                <div className="flex justify-between items-center">
                  <span className="flex items-center gap-1"><Gauge className="h-4 w-4"/> Mileage</span>
                ...
                <div className="flex gap-2 mt-2">
                  <Button variant="outline" size="sm" onClick={() => handleEdit(v)} className="flex-1 flex items-center gap-1">
