...
const getStockStatus = (qty: number, reorder: number) => {
  if (qty === 0) return { label: "Out of Stock", className: "bg-destructive/20 text-destructive" };
  if (qty <= reorder) return { label: "Low Stock", className: "bg-warning/20 text-warning" };
  ...
  <Plus className="mr-2 h-4 w-4" /> Add Part
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card className="bg-card/50 bg-white/10">
            ...
            <CardContent><div className="text-4xl font-bold text-foreground">{items.length}</div></CardContent>
          </Card>
          <Card className="bg-card/50 bg-white/10">
            <CardHeader className="pb-3"><CardTitle className="text-sm text-muted-foreground">Low Stock Alerts</CardTitle></CardHeader>
            <CardContent><div className="text-4xl font-bold text-warning"> {items.filter(i => i.quantity <= i.reorder_level).length}</div></CardContent>
          </Card>
          <Card className="bg-card/50 bg-white/10">
            <CardHeader className="pb-3"><CardTitle className="text-sm text-muted-foreground">Total Value</CardTitle></CardHeader>
            <CardContent><div className="text-4xl font-bold text-primary">₹{items.reduce((sum, i) => sum + i.unit_price * i.quantity, 0).toFixed(2)}</div></CardContent>
          </Card>
        </div>
          ...
          <Card className="border-warning/50 bg-warning/5">
            ...
            <CardContent className="space-y-2">
              {items.filter(i => i.quantity <= i.reorder_level).map((part) => (
                <div key={part.id}className="flex items-center justify-between text-sm">
                  <span className="font-medium text-foreground">{part.name}</span>
                  <span className="text-muted-foreground">Only {part.quantity} left (Reorder at {part.reorder_level})</span>
                </div>
              ))}
            </CardContent>
          </Card>
        ...
        <div className="relative">
          ...
          <Input placeholder="Search parts by name or SKU..." className="pl-10 bg-secondary/50 bg-white/10" value={search}
        ...
        <Card className="bg-card/50 bg-white/10">
          ...
          <CardContent className="space-y-3">
            {items
              ...
              .map((part) => {
              const status = getStockStatus(part.quantity, part.reorder_level);
              return (
                <div key={part.id} className="flex items-center justify-between p-4 bg-secondary/30 border bg-white/10 rounded-xl hover:border-primary/30 transition-all">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-muted-foreground">SKU: {part.part_code} | {part.category}</div>
                    </div>
                  </div>
                  <div className="flex items-center gap-6">
                    <div className="text-right">
                      <div className="text-2xl font-bold text-foreground">{part.quantity}</div>
                      <div className="text-xs text-muted-foreground">in stock</div>
                    </div>
                    <div className="text-right">
                      <div className="font-bold text-primary">₹{part.unit_price}</div>
                    </div>
